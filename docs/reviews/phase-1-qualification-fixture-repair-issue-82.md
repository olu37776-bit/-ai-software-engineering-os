...
The first repaired Windows HEAD `43ea7961f2595a30c7d29e1b3e7938e34cfd29aa` passed the native host-death and all registry tests. A separate lock-election round returned zero winners, so its overall qualification remains FAIL. The existing lock assertion now includes each rejection and fixture-local filesystem failures for diagnosis. It still requires exactly one winner and 15 losers; no runtime behavior or assertion is relaxed. Any necessary runtime repair belongs to a separate O06 operation.
