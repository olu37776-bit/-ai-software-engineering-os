...
| SideEffectTask | 外部非确定性/副作用任务 | 不是 Domain Event |
| SideEffect Worker | 执行 SideEffectTask 的受治理进程/沙箱 | 不拥有状态 transition |
| PersistenceWorker | Runtime 内部专用 storage worker thread | 只执行持久化 batch，不是 SideEffect Worker |
...
| Local Control API | CLI/UI/launcher 控制本地 Runtime 的唯一公开协议 | 不允许直接 DB/Kernel internal path |
| ControlOperation | 由 Control API 创建/查询的 durable application operation | HTTP connection 断开不等于取消 |
...
| PolicySet | 经 Schema 验证的声明式规则集合 | canonical form 为 JSON，不是可执行脚本 |
| PolicySnapshot | 某次决策使用的不可变 Policy 版本 | 供 audit/replay |
| PolicyDecision | canonical evaluator 对 action 的 allow/deny/requirements 结果 | 与 Verification GateDecision 不同 |
| PolicyOverride | 有 scope、expiry、approval 的显式策略放宽事实 | 不能覆盖 hard invariant |
| IsolationLevel | SideEffectTask 所需的可验证执行隔离最低等级 | 不是 Adapter 自报标签 |
| IsolationEvidence | 实际 provider/OS/capability probe 的证据 | 支持 selected isolation claim |
...
| GateDecision | Verification System 形成的权威门禁结果 | Agent 不可自行提交 VERIFIED |
...
`HOST_UNRESTRICTED` 不是可满足 Contract 的 isolation level。完整保证见 ADR-0010。
...
- 用“Worker”不区分 SideEffect Worker 与 PersistenceWorker；
- 用“sandbox”笼统覆盖普通子进程、AppContainer、container 与 remote executor；
- 用“Policy 通过”混淆 PolicyDecision 与 Verification GateDecision；
