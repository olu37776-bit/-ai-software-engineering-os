...
async function assertCanonicalNoReparse(path: string, kind: "file" | "directory"): Promise<string> {
  ...
  const pathSpellingDiffers =
    normalize(canonical).toLowerCase() !== normalize(absolute).toLowerCase();
  if (
    pathSpellingDiffers &&
    (process.platform !== "win32" ||
      sourceStat.dev !== canonicalStat.dev ||
      sourceStat.ino !== canonicalStat.ino)
  ) {
    throw new Error(`${kind} path does not resolve to one stable filesystem identity: ${path}`);
  }
  if (
...
async function validateRequest(
  ...
  readonly canonicalStagingRoot: string;
}> {
  ...
  const canonicalStagingRoot = await assertCanonicalNoReparse(request.stagingRoot, "directory");
  return { canonicalExecutable, canonicalStagingRoot };
}
...
async function createStagingDirectory(
  ...
  const stagingDirectory = await mkdtemp(join(canonicalStagingRoot, "aseos-process-"));
...
async function invokeBridge(
  ...
  return await new Promise((resolvePromise, reject) => {
    ...
    // Register first, then re-check. This closes both sides of the race where cancellation
    // happens after the caller's preflight check but before (or while) the listener is added.
...
export async function runWindowsProcessRestricted(
  ...
  try {
    const canonicalPaths = await validateRequest(request, tool);
