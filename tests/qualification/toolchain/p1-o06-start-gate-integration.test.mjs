...
const baseCommit = "d05c7d85fcf281453329cc2e2f561a1031872376";
const gatePath =
...
async function makeRepository() {
  const parent = await mkdtemp(join(tmpdir(), "aseos-issue64-"));
  temporaryDirectories.push(parent);
  const repository = join(parent, "repo");
  runPass("git", ["clone", "--quiet", "--no-hardlinks", root, repository], parent);
  git(repository, "config", "user.name", "Issue 64 test");
  git(repository, "config", "user.email", "issue64-test@example.invalid");
  ...
  git(repository, "commit", "--quiet", "-m", "test: install Issue 64 transition enforcement");
  return { repository, enforcementBase: git(repository, "rev-parse", "HEAD") };
}

async function createP1O06Operation(repository, operationBase, branch, suffix) {
  git(repository, "checkout", "--quiet", "-B", branch, operationBase);
  const executionPath = `operations/phase-1/executions/p1-o06-${suffix}.json`;
  await writeJson(join(repository, executionPath), {
    schemaVersion: "1.0.0",
    executionId: `P1-O06-${suffix.toUpperCase()}`,
    executionType: "P1_O06_CONTROL_API_TRANSITION",
    operationId: "P1-O06",
    writeScopeOperationId: "P1-O06",
    implementationBranch: branch,
    baseCommit: operationBase,
  });
  const probePath = "packages/contracts/examples/control-api/issue64-probe.json";
  await mkdir(join(repository, "packages/contracts/examples/control-api"), { recursive: true });
  await writeJson(join(repository, probePath), { issue64Probe: true });
...
describe("Issue #64 P1-O06 executable start Gate routing", () => {
  test("fails closed without a legal P1-O06 operation context", async () => {
    const { repository, enforcementBase } = await makeRepository();
    const branch = "phase-1/test-p1-o06-missing-operation-context";
  ...
  test("fails the real operation-aware preflight closed for an absent or malformed Gate", async () => {
    ...
    const absentBranch = "phase-1/test-p1-o06-absent-gate";
    const absentHead = await createP1O06Operation(
      repository,
      enforcementBase,
      absentBranch,
      "issue64-absent-gate",
    ...
    expect(absent.stderr).toContain("P1_O06_START_BLOCKED");
    ...
    git(repository, "commit", "--quiet", "-m", "test: malformed P1-O06 Gate");
    const malformedBase = git(repository, "rev-parse", "HEAD");
    const malformedBranch = "phase-1/test-p1-o06-malformed-gate";
    const malformedHead = await createP1O06Operation(
      repository,
      malformedBase,
      malformedBranch,
      "issue64-malformed-gate",
    );
    const malformed = runScope(repository, malformedBase, malformedHead, malformedBranch);
    expect(malformed.status).toBe(1);
    expect(malformed.stderr).toContain(
      "P1_O06_START_BLOCKED: Issue #64 independent PASS Gate is missing or invalid",
    );
  }, 30_000);
});
