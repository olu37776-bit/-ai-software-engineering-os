...
async function waitForReadyMarker(path, timeoutMs = 50_000) {
  ...
  throw new Error(`Timed out waiting for fixture readiness marker: ${path}`);
}
...
describe.skipIf(process.platform !== "win32")("Windows Job Object security qualification", () => {
  ...
  test("cancels promptly by terminating the complete Job Object", async () => {
    ...
    const readyMarker = join(fixture.outputRoot, `cancel-ready-${randomUUID()}.txt`);
    try {
      const pending = run({
        argv: ["tree-root", readyMarker],
        inputs: [],
        signal: controller.signal,
        limits: defaultLimits({ maxWallClockMs: 20_000, maxProcessCount: 16 }),
      });
      await Promise.race([
        waitForReadyMarker(readyMarker),
        pending.then((result) => {
          throw new Error(
            `Fixture exited before publishing its readiness marker: ${JSON.stringify(result)}`,
          );
        }),
      ]);
